...
- Markdown/TXT 导出
- 商业化 Beta 生产部署骨架
- PostgreSQL schema 与 Docker Compose 开发配置
...
> 当前运行时存储实现仍是 JSON store；PostgreSQL schema 已预留，但切换前需要补齐实际 PostgreSQL store 实现和迁移验证。

## 商业化 Beta 部署

生产部署骨架位于 `infra/docker-compose.prod.yml`，前端镜像会构建静态资源并通过 Nginx 代理 `/api` 到后端。
...
生产环境建议在前端容器前增加 HTTPS 反向代理或云负载均衡。
